...
import 'package:roozaneh/features/app_update/data/app_update_data_providers.dart';
  ...
  Future<AppUpdateState> check() async {
    ...
    return ref
        ...
        .match(
          ...
          (remote) {
            if (remote == null) {
              loggy.info("no remote version available");
              return state = const AppUpdateState.notAvailable();
            }
            try {
              Version? latestVersion;
              try {
                latestVersion = Version.parse(remote.version);
              } catch (_) {}
